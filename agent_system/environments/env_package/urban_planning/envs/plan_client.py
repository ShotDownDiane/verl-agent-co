...
from . import city_config
from .utils import load_yaml, load_pickle, simplify_by_angle, simplify_by_distance, get_boundary_edges, \
    slice_polygon_from_edge, slice_polygon_from_corner, get_intersection_polygon_with_maximum_area
from .utils import set_land_use_array_from_dict
...
class PlanClient(object):
    ...
    def __init__(self, objectives_plan_file: Text, init_plan_file: Text) -> None:
        ...
        """
        file_path = '/home/zhangwentao/myspace/verl-agent-urban/agent_system/environments/env_package/urban_planning/cfg/**/{}.yaml'.format(objectives_plan_file)
        self.objectives = load_yaml(file_path)
        file_path = '/home/zhangwentao/myspace/verl-agent-urban/agent_system/environments/env_package/urban_planning/cfg/**/{}.pickle'.format(init_plan_file)
    ...
    def get_region_graph(self) -> Tuple[nx.Graph, pd.DataFrame]:
        """Returns the UST graph with edge weights based on centroid distances."""
        # 过滤 type 在 [4, 12] ∪ {13} 之间的实体
        regions = self._gdf[
            self._gdf['type'].isin([1] + list(range(4, 13))) & (self._gdf['existence'] == True)
        ]

        # 生成 fuzzy_contiguity 权重
        w = libpysal.weights.fuzzy_contiguity(regions)

        # to_networkx() 默认转换索引为 0,1,2... 需要手动重命名
        graph = w.to_networkx()

        # 创建索引映射: 原始索引 -> to_networkx() 生成的索引
        index_map = dict(enumerate(regions.index))  # 旧索引 -> 新索引
        graph = nx.relabel_nodes(graph, index_map)

        # 计算 geometry 的中心点（centroid）
        centroids = regions.geometry.centroid  # 计算所有实体的中心点

        # 计算中心点之间的最短距离并更新权重
        for u, v in graph.edges():
            centroid_u = centroids.loc[u]
            centroid_v = centroids.loc[v]

            # 计算中心点之间的欧几里得距离
            centroid_distance = centroid_u.distance(centroid_v)

            # 更新图中的边权重
            graph[u][v]['weight'] = centroid_distance

        existing_gdf = self._gdf[self._gdf['existence'] == True]
        return graph, copy.deepcopy(existing_gdf)
    ...
    def _filter_block_by_rule(self,
        ...
        if land_use_type == city_config.SCHOOL:
            ...
            school = gdf[(gdf['type'] == city_config.SCHOOL) | (gdf['type'] == city_config.HOSPITAL_L) | (
                        gdf['type'] == city_config.HOSPITAL_S)].unary_union
    ...
    def get_current_land_use_and_mask(self, land_use_type=None) -> Tuple[Dict, np.ndarray]:
        ...
        land_use_type = land_use_type if land_use_type else \
        self._plan_order[np.logical_or(remaining_plan_area > self.EPSILON, remaining_plan_count > 0)][0]
        land_use['type'] = land_use_type
        mask = self._get_graph_edge_mask(land_use_type)
        land_use['x'] = 0.5
        land_use['y'] = 0.5
        land_use['area'] = self._required_min_area[land_use_type]
