...
class CityEnv:
    ...
    def _get_land_use_and_mask(self, land_use_type=None):
        """
        ...
        if self._stage != 'land_use':
            ...
            land_use, mask = self._plc.get_current_land_use_and_mask(land_use_type=land_use_type)
    ...
    def _get_obs(self, land_use_type=None) -> List:
        """
        ...
        """
        if land_use_type:
            land_use, _ = self._get_land_use_and_mask(land_use_type=land_use_type)
        else:
            land_use = self._current_land_use
        return self._observation_extractor.get_obs(land_use,
    ...
    def step(self, action: np.ndarray, land_use_type=None) -> Tuple[List, float, bool, dict]:
        """
        ...
        Args:
            ...
            land_use (np.ndarray of size 2): The land use.
        ...
        """
        if self._done:
            print('Action taken after episode is done.')
            return self._get_obs(), 0, True,True, {}

        if self._stage == 'land_use':
            if land_use_type:
                land_use_type, _ = self._get_land_use_and_mask(land_use_type=land_use_type)
            else:
                land_use_type = self._current_land_use
            action = int(action[0])
            self._action_history.append((land_use_type, action))
            if self._current_land_use_mask[action] == 0:
                raise InfeasibleActionError(action, self._current_land_use_mask)

            try:
                self.place_land_use(land_use_type, action)
            except Exception as err:
                raise Exception('Actions took before failing to build road: {}'.format(err))
            ...
            if not self._land_use_done and not np.any(self._current_land_use_mask):
                raise Exception('Actions took before becoming infeasible')
