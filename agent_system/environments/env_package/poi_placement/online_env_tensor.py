...
def initial_solution(my_config_dict, my_node_list, s_pos):
    ...
    for key in key_list:
        if key > W:
            break
    best_config = my_config_dict[key].copy()
    return best_config
...
class StationPlacement(gym.Env):
    ...
    def step(self, my_action):
        ...
        if chosen_node in free_list_zero:
            # build new station
            # default_config = initial_solution(self.config_dict, self.node_list, chosen_node, self.budget)
        ...
        if self.schritt >= len(self.node_list) / 2:
            self.game_over = True
        new_score, benefit, cost, charge_time, wait_time, travel_time = ef.norm_score(self.plan_instance.plan, self.node_list,
                                                self.plan_instance.norm_benefit, self.plan_instance.norm_charg,
                                                self.plan_instance.norm_wait, self.plan_instance.norm_travel)
        benefit_info = {
            "score" : new_score,
        ...
        return obs, reward, self.game_over, False, {"benefit_info": benefit_info}
    ...
    def evaluation(self):
        ...
        new_score, benefit, cost, charge_time, wait_time, travel_time = ef.norm_score(self.plan_instance.plan, self.node_list,
                                                 self.plan_instance.norm_benefit, self.plan_instance.norm_charg,
                                                 self.plan_instance.norm_wait, self.plan_instance.norm_travel)
        print("benefit: {}, cost: {}, charge_time: {}, wait_time: {}, travel_time: {}".format(
            benefit, cost, charge_time, wait_time, travel_time
        ))
