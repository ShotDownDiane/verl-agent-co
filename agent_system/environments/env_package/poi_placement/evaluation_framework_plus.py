...
def installment_fee(my_station):
    ...
    fee = s_pos[1]['estate price'] + charger_cost
    s_dict["fee"] = fee  # [fee] = €
    return my_station
...
def node_coverage(my_plan, my_node):
    ...
    single_benefit = I_2 * (1- priv_CS) # changed
...
def norm_score(my_plan, my_node_list, norm_benefit, norm_charg, norm_wait, norm_travel):
    ...
    # cost = cost_travel + charg_time + wait_time  # dimensionless
    # my_score = benefit - cost
    my_score = benefit - (1 - my_lambda) * cost
