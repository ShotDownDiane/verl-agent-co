...
import warnings
import json
from . import my_graph_helpers as mgh
import pandas as pd
...
class MyGraph(object):
    ...
    def save_step_data(self):
        path = '~/verl-agent-urban/agent_system/environments/env_package/road_planning/saves/data.csv'
        if not os.path.exists('~/verl-agent-urban/agent_system/environments/env_package/road_planning/saves/'):
            os.makedirs('~/verl-agent-urban/agent_system/environments/env_package/road_planning/saves/')
    ...
    def get_f2f_avgdist(self):
        sum = 0
        for i in self.inner_facelist:
            for j in self.inner_facelist:
                sum += self.td_dict_face[i][j]
        f2f_avg = sum / (len(self.inner_facelist) * (len(self.inner_facelist) - 1))
        return f2f_avg
        ...
        for pair in change_node:
            ...
            if n1 in self.outerface.nodes:
                for f2 in [f for f in self.inner_facelist if n2 in f.nodes]:
                    self.td_dict_face[self.outerface][f2] = min(self.td_dict_face[self.outerface][f2],
                                                                self.td_dict[idx1][idx2])
            elif n2 in self.outerface.nodes:
                for f1 in [f for f in self.inner_facelist if n1 in f.nodes]:
                    self.td_dict_face[self.outerface][f1] = min(self.td_dict_face[self.outerface][f1],
                                                                self.td_dict[idx1][idx2])
        ...
        edge_width = [
            1.5 * new_road_width if e.road else 1.5 *
                                                new_road_width if e.barrier else 1.5 *
                                                                                 new_road_width if e.interior else 1 for
            e in self.myedges()
        ]
