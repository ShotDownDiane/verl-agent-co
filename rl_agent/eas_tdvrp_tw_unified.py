...
from rl4co.models.zoo.matnet.matnet_time import MatNetTimePolicy
from tensordict import TensorDict

# Import from unified training script
try:
    from train_tdvrp_tw_unified import TDVRPMpVRPMatNetWrapper, TDVRPPolicy, TDVRPEnv, get_paths
except ImportError:
    # Fallback if import fails
    from rl4co.envs.routing.tdvrp.env import TDVRPEnv
    from rl4co.envs.routing.tdtsp.embeddings import TDTSPInitEmbedding, TDTSPContext
...
def run_eas():
    parser = argparse.ArgumentParser(description="Unified EAS Script for TDVRP-TW")
    ...
    if args.ckpt_path is None:
        candidates = [
            f"ckpt/tdvrptw/{args.model}/{args.city}_{args.size}.ckpt",
            f"tdvrptw_{args.model}_{args.city}_{args.size}.ckpt",
            f"last.ckpt"
        ]
    ...
    valid_env = EnvClass(
        generator_params=valid_generator_params,
        penalty_value=0.0
    )
    ...
    num_samples = min(valid_env.generator.num_samples, 500)
    ...
    num_starts = args.size if args.model == "pomo" else 0
    ...
    max_eas_samples = 100 # Limit for demo
    ...
    for i in range(train_steps):
        ...
        for step in range(args.eas_steps):
            ...
            if args.model == "pomo":
                # POMO Logic
                out = policy(td_init.clone(), train_env, decode_type="multistart_greedy", num_starts=num_starts)
    ...
    # Final Validation
    log.info("Running Final Validation on Test Set...")
    valid_env.generator.phase = "all"
    valid_env.generator._load_data()
    valid_env.generator.random_sample = False
    valid_env.generator.current_instance_id = 0
    ...
    log.info(f"Final Validation Reward: {avg_val_reward_final:.4f}")
    log.info(f"Average Inference Time per Instance: {avg_time_per_instance:.4f}s")
    
    swanlab.log({
        "final_val_reward": avg_val_reward_final,
