...
class Agent:
    ...
    def get_action_from_gpt(self, obs):
        ...
        # 步骤 2: 组合观察和计划
        combined_input = f"观察: {obs}\n计划: {plan}\n根据上述观察和计划，确定下一步行动是什么？切记最终的回答也需要使用<action></action>标签进行包裹。"
...
if __name__ == "__main__":
    # -------- logging ----------
    os.makedirs(f"logs/{sys.argv[1]}/urban_planning", exist_ok=True)
    log_fp = os.path.join(
        f"logs/{sys.argv[1]}/urban_planning", f"run_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    )
    logging.basicConfig(
        level=logging.INFO,
