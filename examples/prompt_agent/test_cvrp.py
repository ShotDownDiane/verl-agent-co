...
from agent_system.environments.env_package.rl4co.route_envs import RouteWorker, RouteEnvs
...
def run_agent_loop(env_manager, agent, solution_tour=None, env_name="cvrp", instance_idx=0, heuristic_objs=None):
    """
    Refactored agent loop using RL4COEnvironmentManager for batch processing and prompt handling.
    """
    ...
    while True:
        ...
        images = observations.get('image', None) # List of numpy arrays or paths
        ...
        if np.all(dones):
            ...
            # Calculate and report objective and gap
            if heuristic_objs is not None:
                # Assuming reward scale is 0.1 as per config
                reward_scale = 0.1
                ...
                if batch_gaps:
                    ...
                    metrics['gap'] = avg_batch_gap # Store average for the batch
...
def main():
    ...
    if 'cvrp' not in routing_data:
        print("No CVRP data found.")
        return
    ...
    print("\n" + "="*50)
    print("1. Single Worker Execution (CVRP) with RL4COEnvironmentManager")
    print("="*50)

    cvrp_data = routing_data['cvrp']
    n = len(cvrp_data)
    if n == 0:
        print("No CVRP instances.")
        return
    ...
    for i in range(0, num_to_run, batch_size):
        ...
        batch_data = cvrp_data[i : i + current_bs]
        ...
        # Mock Config for RL4COEnvironmentManager
        ...
        metrics = run_agent_loop(env_manager, agent, solution_tour=None, env_name="cvrp", instance_idx=i, heuristic_objs=heuristic_objs)
