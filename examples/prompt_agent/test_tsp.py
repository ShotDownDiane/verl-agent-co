...
class LoadedDataGenerator:
    ...
    def __call__(self, batch_size):
        ...
        print(f"current process {self.idx}/{len(self.data_list)}")
...
def run_agent_loop(env_manager, agent, solution_tour=None, env_name="tsp", instance_idx=0, heuristic_objs=None):
...
def main():
    ...
    if 'tsp' not in routing_data:
        print("No TSP data found.")
        return
    ...
    print("\n" + "="*50)
    print("1. Single Worker Execution (TSP) with RL4COEnvironmentManager")
    print("="*50)

    tsp_data = routing_data['tsp']
    n = len(tsp_data)
    if n == 0:
        print("No TSP instances.")
        return
    ...
    for i in range(0, num_to_run, batch_size):
        ...
        batch_data = tsp_data[i : i + current_bs]
        ...
        metrics = run_agent_loop(env_manager, agent, solution_tour=None, env_name="tsp", instance_idx=i, heuristic_objs=heuristic_objs)
