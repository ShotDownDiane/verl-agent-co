        ...
        if solution_tour is not None:
            try:
                ...
                if target_cand is None and len(solution_set) > 0 and len(avail_candidates) > 0:
                    ...
                    # ------------------------------------------
                    # 分支 1: GSTP (基于边的中点距离比较)
                    # ------------------------------------------
                    if global_edge_list is None:
                            raise ValueError("Edge list not found for STP task.")
                    
                    # 辅助函数：计算边的几何中点
                    def get_edge_midpoint(edge_idx, edge_list, coords):
                        idx = int(edge_idx)
                        # 获取端点 ID
                        u, v = edge_list[idx][0], edge_list[idx][1]
                    ...
                    # O(K*M) 双重循环寻找最相似的“替身”
                    for k_idx, c_edge_idx in enumerate(avail_candidates):
                        c_mid = get_edge_midpoint(c_edge_idx, global_edge_list, all_coords)
                        
                        for o_edge_idx in remaining_opt_items:
                            o_mid = get_edge_midpoint(o_edge_idx, global_edge_list, all_coords)
                            
                            # 计算距离
                            if isinstance(c_mid, torch.Tensor):
                                dist = torch.norm(c_mid - o_mid).item()
                            else:
                                dist = np.linalg.norm(c_mid - o_mid)
                            
                            if dist < min_dist:
                                min_dist = dist
                                best_swap_pair = (k_idx, o_edge_idx)
                    ...
                    if best_swap_pair is not None:
                        ...
                        COUNT += 1
                        print(f"COUNT: {COUNT}")
                        print(f"Modified Acts: {envs._td['topk_acts'][0]}")
                        
                        obs_new = build_obs_stp(envs._td, 1, given_topk_acts= [acts_container], image_obs=True)
...
def main():
    ...
    print("\n" + "="*50)
    print("1. Single Worker Execution (STP)")
    print("="*50)
