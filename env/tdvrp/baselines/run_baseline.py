...
from rl4co.envs.routing.tdvrp.baselines.funsearch import solve_funsearch
from rl4co.envs.routing.tdvrp.baselines.greedy_refine import solve_greedy_refine
...
def solve_instance(args):
    ...
    try:
        ...
        elif method == "greedy_refine":
            routes, cost = solve_greedy_refine(
                matrix=matrix,
                duration=duration,
                time_windows=time_windows,
                service_times=service_times,
        ...
        if not routes:
             return float('inf'), end_t - start_t, 0, 0, None
...
def main():
    ...
    parser.add_argument("--method", type=str, default="sa", choices=["greedy", "sa", "sah", "aco", "ortools", "alns", "grasp", "funsearch", "greedy_refine", "tabu", "mip"], help="Solver method")
    ...
    if method in ["funsearch", "greedy_refine", "mip"]:
        num_workers = 1 # Reduce workers for LLM-based methods to avoid Rate Limit
    else:
        num_workers = 1
    ...
    SOLVER_CONFIGS = {
        ...
        "sa": {
            ...
            "slow": {"iterations": 5000, "initial_temp": 1000.0}
        },
        "sah": {
            "fast": {"iterations": 200, "initial_temp": 100.0},
            "slow": {"iterations": 5000, "initial_temp": 1000.0}
        ...
        "funsearch": {
            "fast": {"iterations": 10, "timeout": 360},
            "slow": {"iterations": 20, "timeout": 360}
        },
        "greedy_refine": {
            "fast": {"iterations": 5, "timeout": 120},
            "slow": {"iterations": 20, "timeout": 30}
        },
        "tabu": {
            "fast": {"iterations": 200},
    ...
    # --- Initialize SwanLab ---
    if method in ["funsearch", "greedy_refine"] and not os.environ.get("OPENAI_API_KEY") and not os.path.exists("openai_key.txt"):
        print(f"\nWARNING: OPENAI_API_KEY not found in environment and openai_key.txt does not exist.")
        print(f"{method} will skip execution or fail. Please set the API key.\n")

    swanlab.init(
        project="TDVRP-Baselines-0129",
                ...
                try:
                    ...
                    for i in range(actual_samples):
                        ...
                        if isinstance(service_times, (int, float)):
                            service_times = np.full(matrix.shape[0], service_times)
